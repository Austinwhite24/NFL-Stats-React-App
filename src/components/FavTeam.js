...
// axios.get(`http://localhost:3000/test`).then(function (response) {console.log(response.data)})
...
// }, []);
    return (
      ...
      <div className="favTeamsDiv">
        ...
        |<Link className='rightLinks'to="/home">Home</Link>
        |<Link className='rightLinks'to="/register">Register</Link>
        {/* |<Link className='rightLinks'to="/logout">Logout</Link> */}
      </div>
      </header>

      <div className="logoutDiv">
        <button className="logoutBtn" role='button' onClick={logout}>Logout</button>
      </div>
        <main>
